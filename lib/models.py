        ...
        if args.NN_conv:
            ...
            if args.spectral_G:
                main.add_module('End-SpectralConv2d', torch.nn.utils.spectral_norm(
                    torch.nn.Conv2d(args.G_h_size, args.n_colors, kernel_size=3, stride=1, padding=1)))
            else:
            ...
            if args.spectral_G:
                main.add_module('End-SpectralConvTranspose2d', torch.nn.utils.spectral_norm(
                    torch.nn.ConvTranspose2d(args.G_h_size, args.n_colors, kernel_size=4, stride=2, padding=1,
                                             bias=False)))
            else:
                main.add_module('End-ConvTranspose2d',
                                torch.nn.ConvTranspose2d(args.G_h_size, args.n_colors, kernel_size=4, stride=2,
    ...
    def __init__(self, args):
        ...
        if args.spectral:
            main.add_module('Start-SpectralConv2d', torch.nn.utils.spectral_norm(
                torch.nn.Conv2d(args.n_colors, args.D_h_size, kernel_size=4, stride=2, padding=1, bias=False)))
        else:
            main.add_module('Start-Conv2d',
                            torch.nn.Conv2d(args.n_colors, args.D_h_size, kernel_size=4, stride=2, padding=1,
                                            bias=False))
        if args.SELU:
            main.add_module('Start-SELU', torch.nn.SELU(inplace=True))
        else:
...
#####################################
class WGenerator28(nn.Module):
    def __init__(self, noise_dim, channels, feature_maps):
        super(WGenerator28, self).__init__()
        self.nz = noise_dim  # Size of z latent vector
        self.ngf = feature_maps  # Size of feature maps in generator
        self.nc = channels  # Number of channels in the training images
        self.network = nn.Sequential(
            ...
            nn.BatchNorm2d(self.ngf*4),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf*4, out_channels=self.nc, kernel_size=1, stride=1, padding=2,
                               bias=False),
            nn.Tanh()
        )
...
class Critic28(nn.Module):
    """
        WGAN-GP Critic model for 28x28-sized images.
    """
    def __init__(self, channels, feature_maps):
        ...
        self.network = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=self.ndf*4, out_channels=1, kernel_size=4, stride=2, padding=1, bias=False),
        )
...
class WGenerator128(nn.Module):
    def __init__(self, noise_dim, channels, feature_maps):
        super(WGenerator128, self).__init__()
        self.nz = noise_dim  # Size of z latent vector
        self.ngf = feature_maps  # Size of feature maps in generator
        self.nc = channels  # Number of channels in the training images
        self.network = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.nz, out_channels=self.ngf*8, kernel_size=6, stride=1, padding=0,
                               bias=False),
            nn.BatchNorm2d(self.ngf*8),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf*8, out_channels=self.ngf*4, kernel_size=6, stride=2, padding=1,
                               bias=False),
            nn.BatchNorm2d(self.ngf*4),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf*4, out_channels=self.ngf*2, kernel_size=6, stride=2, padding=1,
                               bias=False),
            nn.BatchNorm2d(self.ngf*2),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf*2, out_channels=self.ngf, kernel_size=6, stride=2, padding=1,
                               bias=False),
            nn.BatchNorm2d(self.ngf),
            nn.ReLU(True),
