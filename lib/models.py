...
class Generator128(nn.Module):
    """
        Generator model for 128x128-sized images.
    """
    ...
    def __init__(self, noise_dim, channels, feature_maps):
        ...
        self.nc = channels  # Number of channels in the training images
        self.network = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.nz, out_channels=self.ngf * 8, kernel_size=6, stride=1,
                               padding=0, bias=False),
            nn.BatchNorm2d(self.ngf * 8),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf * 8, out_channels=self.ngf * 4, kernel_size=6, stride=2,
                               padding=1, bias=False),
            nn.BatchNorm2d(self.ngf * 4),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf * 4, out_channels=self.ngf * 2, kernel_size=6, stride=2,
                               padding=1, bias=False),
            nn.BatchNorm2d(self.ngf * 2),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf * 2, out_channels=self.ngf, kernel_size=6, stride=2,
                               padding=1, bias=False),
            nn.BatchNorm2d(self.ngf),
            nn.ReLU(True),
            nn.ConvTranspose2d(in_channels=self.ngf, out_channels=self.nc, kernel_size=6, stride=2,
                               padding=1, bias=False),
            nn.Tanh()
        )
...
class Discriminator128(nn.Module):
    """
        DCGAN Discriminator model for 128x128-sized images.
    """
    ...
    def __init__(self, channels, feature_maps):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(in_channels=self.nc, out_channels=self.ndf, kernel_size=6, stride=2, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=self.ndf, out_channels=self.ndf * 2, kernel_size=6, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=self.ndf * 2, out_channels=self.ndf * 4, kernel_size=6, stride=2, padding=1,
                      bias=False),
            nn.BatchNorm2d(self.ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=self.ndf * 4, out_channels=self.ndf * 8, kernel_size=6, stride=2, padding=1,
                      bias=False),
            nn.BatchNorm2d(self.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
...
class Critic256(nn.Module):
    ...
    def __init__(self, channels, feature_maps):
        ...
        self.network = nn.Sequential(
            ...
            nn.InstanceNorm2d(self.ndf * 2, affine=True),
            nn.LeakyReLU(0.2),
            nn.Conv2d(in_channels=self.ndf * 2, out_channels=self.ndf * 4, kernel_size=10, stride=2, padding=1,
                      bias=False),
            nn.InstanceNorm2d(self.ndf * 4, affine=True),
            nn.LeakyReLU(0.2),
            nn.Conv2d(in_channels=self.ndf * 4, out_channels=self.ndf * 8, kernel_size=10, stride=2, padding=1,
                      bias=False),
            nn.InstanceNorm2d(self.ndf * 8, affine=True),
            nn.LeakyReLU(0.2),
